...
func (suite *RadosTestSuite) TestChecksum() {
	...
	suite.Run("CRC32", func() {
		...
		dst := make([]byte, 4+4)
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeCRC32C, dst, &ChecksumOptions{InitValue: init}))
	...
	suite.Run("XXHash32", func() {
		...
		dst := make([]byte, 4+4)
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeXXHash32, dst, nil))
	...
	suite.Run("XXHash64", func() {
		...
		dst := make([]byte, 4+8)
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeXXHash64, dst, nil))
...
func (suite *RadosTestSuite) TestChecksumWithOpts() {
	...
	suite.Run("CRC32", func() {
		...
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeCRC32C, dst, &ChecksumOptions{
			InitValue: init,
			ChunkSize: chunkSize,
			Off:       off,
	...
	suite.Run("XXHash32", func() {
		...
		dst := make([]byte, 4+4*chunkCount) // allocate space for multiple uint32 checksum values
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeXXHash32, dst, &ChecksumOptions{
			InitValue: init,
			ChunkSize: chunkSize,
			Off:       off,
			Len:       chunkSize * chunkCount,
	...
	suite.Run("XXHash64", func() {
		...
		dst := make([]byte, 4+8*3) // allocate space for multiple uint64 checksum values
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeXXHash64, dst, &ChecksumOptions{
			InitValue: init,
			ChunkSize: chunkSize,
			Off:       off,
			Len:       chunkSize * chunkCount,
