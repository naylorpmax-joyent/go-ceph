...
func (suite *RadosTestSuite) TestChecksum() {
	...
	suite.Run("CRC32", func() {
		...
		dst := make([]byte, 4+4)
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeCRC32C, dst, ChecksumInitValue(init)))
	...
	suite.Run("XXHash32", func() {
		...
		dst := make([]byte, 4+4)
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeXXHash32, dst))
	...
	suite.Run("XXHash64", func() {
		...
		dst := make([]byte, 4+8)
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeXXHash64, dst))
...
func (suite *RadosTestSuite) TestChecksumWithOpts() {
	...
	suite.Run("CRC32", func() {
		...
		assert.Nil(suite.T(), suite.ioctx.Checksum(oid, ChecksumTypeCRC32C, dst,
			ChecksumInitValue(init), ChecksumChunkSize(chunkSize), ChecksumOff(off), ChecksumLen(chunkSize*chunkCount),
		))
